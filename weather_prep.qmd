...
labels <- bind_rows(
  ...
  mutate(
    ...
    nudge_x_icon = c(0.093, 0.052, 0.097, 0.041, -0.087),
    nudge_y_icon = c(0.099, -0.115, -0.078, 0.114, 0.053),
    nudge_x_repel = c(0.07, 0.033, 0.061, 0.008, -0.135),
    nudge_y_repel = c(0.078, -0.096, -0.061, 0.12, 0.03)
  )
...
ggplot() +
  ...
  scale_fill_fermenter(palette = "Blues", name = "Population Count", breaks = seq(0, 500000, by = 50000), labels = scales::comma, direction = 1) +
  scale_color_manual(values = event_colors, name = "Event Type") +
  scale_size_continuous(range = c(10,20), name = "No. of Months") +
...
ggplot() +
  # HDB building data
  geom_sf(data = singapore_map, fill = "#c0c0c0c0", color = "black", size = 0.5) +
  
  geom_sf(data = hdb_buildings, fill = NA, color = "#ff4040b0", size = 0.2) +
